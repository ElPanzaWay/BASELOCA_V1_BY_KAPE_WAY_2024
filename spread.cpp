...
double GetConstantRemainder(const Vector& vecSpread) // aka Trap God Constant
{
...
{
	vecSpread.x = UTIL_SharedRandomFloat(random_seed + future, -0.5f, 0.5f) + 
		          UTIL_SharedRandomFloat(random_seed + 1 + future, -0.5f, 0.5f);
	vecSpread.y = UTIL_SharedRandomFloat(random_seed + 2 + future, -0.5f, 0.5f) + 
		          UTIL_SharedRandomFloat(random_seed + 3 + future, -0.5f, 0.5f);

	//
	// TODO: obtener VECSPREAD
	//
	vecSpread.x *= GetPlayerLocaleVecSpread(); //PlayerLocale.spread.vecspread; //m_flSpread;
	vecSpread.y *= GetPlayerLocaleVecSpread(); //PlayerLocale.spread.vecspread; //m_flSpread;
...
void GetSpreadOffset(const unsigned& random_seed, const int& future, 
					 
					 const QAngle& QInAngles, QAngle& QOutAngles, const int &type)
{
	...
	if (type > NOSPREAD_NONE)// tipo de nospread
	{
		...
		if (a * (vecForward.x * vecForward.x + a - dot * dot) < 0.f && 
			
			(type == NOSPREAD_PITCH_ROLL || type == NOSPREAD_PITCH_YAW_ROLL))
		{
...
void VectorAngles(const Vector &vecForward, QAngle& QAngles)
{
	float flYaw, flPitch;
...
extern float g_aimvector[3];

void ApplyNS(float * cmd_view_angles, int ns_type, bool bApplyOnlyIf)
{
	/*
	Vector vecSrc(GetMyViewPoint());
	QAngle QAimAngles, QNewAngles;
	VectorAngles(g_aimvector - vecSrc, QAimAngles);
	QNewAngles = QAimAngles;
	*/

/*	if (CVARS.no_w_recoil)
	{
		QNewAngles[0] -= client_state->punchangle[0] * 2.f;
		QNewAngles[1] -= client_state->punchangle[1] * 2.f;
	}*/

//	GetSpreadOffset(GetPlayerLocaleRandomSeed(), 1, QNewAngles, QNewAngles, /*CVARS.nospread*/1);
...
//					 const QAngle& QInAngles, QAngle& QOutAngles, const int &type);
	...
	QNewAngles.x = cmd_view_angles[0];
	QNewAngles.y = cmd_view_angles[1];
	QNewAngles.z = cmd_view_angles[2];
	
	GetSpreadOffset(GetPlayerLocaleRandomSeed(), 1, QNewAngles, QNewAngles, ns_type);

	if(bApplyOnlyIf)
	{
	cmd_view_angles[0] = QNewAngles.x;
	cmd_view_angles[1] = QNewAngles.y;
	cmd_view_angles[2] = QNewAngles.z;
	}
