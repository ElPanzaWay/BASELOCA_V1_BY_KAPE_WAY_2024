...
VOID InitHook(){
  if(!pSendTo||!pOrigSendTo){
    HMODULE hWS2=GetModuleHandleA("ws2_32.dll\0");
    if(hWS2){
      pOrigSendTo=(tSendTo)GetProcAddress(hWS2,"sendto\0");
      if(pOrigSendTo){
        ...
        if(pSendTo){
          ...
          *(DWORD*)((PBYTE)pSendTo+SENDTOLEN+1)=(DWORD)((PBYTE)pOrigSendTo-(PBYTE)pSendTo)-5;
            ...
            DbgPrint("*** Error allocating memory for sendto!\0");
          #endif
        }
      }else{
        #if defined(_DEBUG) && !defined(VM_PROTECT)
          DbgPrint("*** Error getting sendto from ws2_32.dll!\0");
        #endif
      }
    }else{
      #if defined(_DEBUG) && !defined(VM_PROTECT)
        DbgPrint("*** Error getting ws2_32.dll!\0");
      #endif
    }
  }
}
...
VOID HookSendTo(){
  ...
  InitHook();
  #if defined(_DEBUG) && !defined(VM_PROTECT)
  DbgPrint("*** Hook SendTo");
  #endif
  if(pSendTo&&pOrigSendTo){
    if(hVirtualProtect((PVOID)pOrigSendTo,SENDTOLEN,PAGE_READWRITE,&dwback)){
      *((PBYTE)pOrigSendTo)=0xE9;
      *(DWORD*)((PBYTE)pOrigSendTo+1)=(DWORD)((PBYTE)hSendTo-(PBYTE)pOrigSendTo)-5;
      hVirtualProtect((PVOID)pOrigSendTo,SENDTOLEN,dwback,&dwback);
    }
  }else{
    #if defined(_DEBUG) && !defined(VM_PROTECT)
      DbgPrint("*** Error buffer not set!");
    #endif
  }
}
...
VOID UnHookSendTo(){
  DWORD dwback=0;
  #if defined(_DEBUG) && !defined(VM_PROTECT)
  DbgPrint("*** UnHook SendTo");
  #endif
  if(pSendTo&&pOrigSendTo&&*((PBYTE)pOrigSendTo)==0xE9){
    hVirtualProtect((PVOID)pOrigSendTo,SENDTOLEN,PAGE_READWRITE,&dwback);
    memmove((PBYTE)pOrigSendTo,(PBYTE)pSendTo,SENDTOLEN);
    hVirtualProtect((PVOID)pOrigSendTo,SENDTOLEN,dwback,&dwback);
  }else{
    #if defined(_DEBUG) && !defined(VM_PROTECT)
      DbgPrint("*** Error buffer not set 2!");
    #endif
  }
}
...
SteamProtocol::SteamProtocol(){
  header=0xFFFFFFFF;
  strcpy(connect,"connect\0");
  strcpy(getchallenge,"getchallenge\0");
  strcpy(steam,"steam\0");
  strcpy(svr,"48\0");
...
bool SteamProtocol::Parse(const char * &buf,int &len){
  if(len>3&&!strncmp(buf,(CHAR*)&header,4)){
    if(len>=11&&!strncmp(buf+4,connect,7)){
      #if defined(_DEBUG) && !defined(VM_PROTECT)
      DbgPrint("*** connect-> len: %d buffer: %.*s",len,len,buf);
      DbgHexDump((PVOID)buf,len);
      #endif
      ...
      #if defined(_DEBUG) && !defined(VM_PROTECT)
      DbgPrint("*** response connect-> len: %d buffer: %.*s",len,len,buf);
      DbgHexDump((PVOID)buf,len);
      #endif
      return false;
    }else if(len>=16&&!strncmp(buf+4,getchallenge,12)){
      #if defined(_DEBUG) && !defined(VM_PROTECT)
      DbgPrint("*** getchallenge-> len: %d buffer: %.*s",len,len,buf);
      DbgHexDump((PVOID)buf,len);
      #endif
      ...
      #if defined(_DEBUG) && !defined(VM_PROTECT)
      DbgPrint("*** response getchallenge-> len: %d buffer: %.*s",len,len,buf);
      DbgHexDump((PVOID)buf,len);
      #endif
    }
  }else
